...
pub trait LiquidLocking {
    ...
    fn whitelist_token(&self, token: TokenIdentifier) {
        require!(token.is_valid_dct_identifier(), "invalid token provided");
    ...
    fn validate_payment(&self, payment: &DctTokenPayment) {
    ...
    fn stake_token(&self, caller: &ManagedAddress, payment: DctTokenPayment) {
    ...
    fn unbond(&self, tokens: ManagedVec<TokenIdentifier>) {
        let caller = self.blockchain().get_caller();
        let block_epoch = self.blockchain().get_block_epoch();
        let mut unbond_tokens = ManagedVec::<Self::Api, DctTokenPayment<Self::Api>>::new();
        for token_identifier in tokens.iter() {
            ...
            if unbond_amount > 0u64 {
                unbond_tokens.push(DctTokenPayment::new(
    ...
    #[event("lock")]
    fn lock_event(&self, #[indexed] user: &ManagedAddress, tokens: &ManagedVec<DctTokenPayment>);

    #[event("unlock")]
    fn unlock_event(&self, #[indexed] user: &ManagedAddress, tokens: &ManagedVec<DctTokenPayment>);
    ...
    fn locked_token_amounts_by_address(
        ...
        let mut amounts = ManagedVec::<Self::Api, DctTokenPayment<Self::Api>>::new();
        for token in self.locked_tokens(&address).iter() {
            let amount = self.locked_token_amounts(&address, &token).get();
            let payment = DctTokenPayment::new(token, 0, amount);
    ...
    fn unlocked_token_by_address(
        ...
        for token in self.unlocked_tokens(&address).iter() {
            for epoch in self.unlocked_token_epochs(&address, &token).iter() {
                ...
                let payment = DctTokenPayment::new(token.clone(), 0, amount);
