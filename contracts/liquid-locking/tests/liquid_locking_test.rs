use liquid_locking::*;
use dharitri_sc::types::{BigUint, DctTokenPayment, ManagedVec, TokenIdentifier};
...
fn test() {
    ...
    world
        .set_state_step(
            SetStateStep::new().put_account(
                user_1,
                Account::new()
                    .dct_balance(whitelisted_token_1, 1_000u64)
                    .dct_balance(whitelisted_token_2, 1_000u64),
            ),
        )
        .set_state_step(
            SetStateStep::new().put_account(
                user_2,
                Account::new()
                    .dct_balance(blacklisted_token, 1_000u64)
                    .dct_balance(whitelisted_token_2, 1_000u64)
    ...
    world
        .sc_call_step(
            ScCallStep::new()
                .from(user_2)
                .to(&contract)
                .dct_transfer(blacklisted_token, 0u64, 500u64)
                .call(contract.lock())
    ...
    world
        .sc_call_step(
            ScCallStep::new()
                .from(user_2)
                .to(&contract)
                .dct_transfer(whitelisted_token_2, 0u64, 500u64)
                .call(contract.lock())
                .expect(TxExpect::ok().no_result()),
        )
        .sc_call_step(
        ...
        .sc_call_step(
            ScCallStep::new()
                .from(user_1)
                .to(&contract)
                .dct_transfer(whitelisted_token_1, 0u64, 1_000u64)
                .dct_transfer(whitelisted_token_2, 0u64, 1_000u64)
                .call(contract.lock())
                .expect(TxExpect::ok().no_result()),
        )
        .check_state_step(
            CheckStateStep::new()
                ...
                .put_account(
                    ...
                    CheckAccount::new()
                        .dct_balance(whitelisted_token_1, 1_000u64)
                        .dct_balance(whitelisted_token_2, 2_000u64)
                        .check_storage("str:unbond_period", "10")
                        .check_storage("str:token_whitelist.len", "2")
                        .check_storage("str:token_whitelist.item|u32:1", "str:AAA-111111")
                        .check_storage("str:token_whitelist.item|u32:2", "str:BBB-222222")
    ...
    let mut unlock_single_dct = ManagedVec::<DebugApi, DctTokenPayment<DebugApi>>::new();
    let mut unlock_multiple_dct = ManagedVec::<DebugApi, DctTokenPayment<DebugApi>>::new();

    unlock_single_dct.push(DctTokenPayment {
        token_identifier: TokenIdentifier::from(blacklisted_token_id),
        token_nonce: 0,
        amount: BigUint::from(1500u64),
    });
    unlock_multiple_dct.push(DctTokenPayment {
    ...
    world
        .sc_call_step(
            ScCallStep::new()
                .from(user_2)
                .to(&contract)
                .call(contract.unlock(unlock_single_dct))
        ...
        .sc_call_step(
            ScCallStep::new()
                .from(user_2)
                .to(&contract)
                .call(contract.unlock(unlock_multiple_dct))
                .expect(TxExpect::err(4, "str:requested amount cannot be 0")),
        );

    // unlock success

    unlock_single_dct = ManagedVec::<DebugApi, DctTokenPayment<DebugApi>>::new();
    unlock_multiple_dct = ManagedVec::<DebugApi, DctTokenPayment<DebugApi>>::new();
    ...
    world
        .sc_call_step(
            ScCallStep::new()
                .from(user_2)
                .to(&contract)
                .call(contract.unlock(unlock_single_dct.clone()))
                .expect(TxExpect::ok().no_result()),
        )
        .sc_call_step(
            ScCallStep::new()
        ...
        .check_state_step(
            CheckStateStep::new()
                ...
                .put_account(
                    ...
                    CheckAccount::new()
                        .dct_balance(whitelisted_token_1, 1_000u64)
                        .dct_balance(whitelisted_token_2, 700u64),
                )
                .put_account(
                    user_2,
                    CheckAccount::new()
                        .dct_balance(blacklisted_token, 1_000u64)
                        .dct_balance(whitelisted_token_2, 200u64)
                        .balance("1_000_000_000"),
                )
                .put_account(
                    &contract,
                    CheckAccount::new()
                        .dct_balance(whitelisted_token_2, 1_100u64)
                        .check_storage("str:unbond_period", "10")
                        .check_storage("str:token_whitelist.len", "2")
                        .check_storage("str:token_whitelist.item|u32:1", "str:AAA-111111")
                        .check_storage("str:token_whitelist.item|u32:2", "str:BBB-222222")
